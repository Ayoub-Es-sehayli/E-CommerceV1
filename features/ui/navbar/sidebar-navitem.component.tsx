...
export default function SideBarNavItem({ category }: NavItemProps) {
  return (
    <li
      tabIndex={0}
    ...
    >
      <label className="flex justify-between items-center">
        <Link href={`/shop/products?category%5B0%5D=${category.name}`}>
          <a className="daisy-collapse-title px-2">{category.name}</a>
        </Link>
        ...
        <ul className="daisy-collapse-content w-auto ml-2 pl-2 border-l-4 border-primary-200 grid gap-1">
          {category.subcategories.map((subcategory) => (
            <li key={subcategory.id}>
              <Link href={`/shop/products?category%5B0%5D=${subcategory.name}`}>
